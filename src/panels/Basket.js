...
const Basket = ({
  ...
  if (localStorage.getItem(itemId)) {
    restInfo = JSON.parse(localStorage.getItem(itemId));
  }

  const [faster, setFaster] = useState(restInfo.faster);
  const [time, setTime] = useState(restInfo.time);
  const [selfService, setSelfService] = useState(restInfo.selfService);
